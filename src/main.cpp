...
#if defined(NDEBUG)
# error "Emptycoin cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "Emptycoin Signed Message:\n";
...
bool IsStandardTx(const CTransaction& tx, string& reason)
{
    ...
    if (tx.nTime > Params().FutureDrift(GetAdjustedTime(), nBestHeight + 1)) {
        reason = "time-too-new";
        return false;
    }
...
int64_t GetProofOfWorkReward(int64_t nFees, int64_t nHeight)
{
    ...
    int64_t nSubsidy = Params().BasicPoWReward();
    int64_t n = nHeight / Params().PoWHalfLife();
    nSubsidy >>= n;
...
{
    Bignum nSubsidy;
    nSubsidy = nCoinAge * Params().StakeCoinYearReward() * 33 / (365 * 33 + 8);
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner, unsigned int flags)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            {
                int nSpendDepth;
                if (IsConfirmedInNPrevBlocks(txindex, pindexBlock, Params().CoinbaseMaturity(), nSpendDepth))
        ...
        if (!IsCoinStake())
        {
            ...
            unsigned int nBytes = GetSerializeSize(SER_NETWORK, CTransaction::CURRENT_VERSION);
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    int64_t nHeight = pindex->pprev->nHeight + 1;
    unsigned int nSigOps = 0;
    BOOST_FOREACH(CTransaction& tx, vtx)
    {
...
bool CTransaction::GetCoinAge(CTxDB& txdb, const CBlockIndex* pindexPrev, uint64_t& nCoinAge) const
{
    ...
    BOOST_FOREACH(const CTxIn& txin, vin)
    {
        ...
        int nSpendDepth;
        if (IsConfirmedInNPrevBlocks(txindex, pindexPrev, Params().StakeMinConfirmations() - 1, nSpendDepth))
            {
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProof)
{
    ...
    pindexNew->bnStakeModifier = ComputeStakeModifier(pindexNew->pprev, IsProofOfWork() ? hashProof : vtx[1].vin[0].prevout.hash);
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    // Check timestamp
    if (GetBlockTime() > Params().FutureDrift(GetAdjustedTime(), 0))
        return error("CheckBlock() : block timestamp too far in the future");

    // Check that block mined after official start
    if (GetBlockTime() < Params().LaunchTime())
        return error("CheckBlock() : mining time violation");
...
bool CBlock::AcceptBlock()
{
    ...
    if (GetBlockTime() > Params().FutureDrift((int64_t)vtx[0].nTime, nHeight))
    ...
    //Check coinstake delay
    if (IsProofOfStake() && (GetBlockTime() - pindexPrev->nTime)< Params().MinDelay())
        return DoS(1, error("AcceptBlock() : block delay violation nTimeBlock=%d nTimePrevBlock=%u", GetBlockTime(), pindexPrev->nTime));
    ...
    if (GetBlockTime() <= pindexPrev->GetPastTimeLimit() || Params().FutureDrift(GetBlockTime(), nHeight) < pindexPrev->GetBlockTime())
...
bool CBlock::SignBlock(CWallet& wallet, int64_t nFees)
{
    ...
    txCoinStake.nTime &= ~Params().PoSGranularity();
...
{
    RenameThread("emptycoin-loadblk");
