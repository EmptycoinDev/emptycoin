...
// Copyright (c) 2014 The Emptycoin developers
...
bool ComputeNextStakeModifier(const CBlockIndex* pindexPrev, uint64_t& nStakeModifier, bool& fGeneratedStakeModifier)
{
    ...
    vSortedByTimestamp.reserve(64 * Params().ModifierInterval() / Params().GetTargetSpacing(pindexPrev->nHeight));
...
//
static bool CheckStakeKernelHash(CBlockIndex* pindexPrev, unsigned int nBits, unsigned int nTimeBlockFrom, const CTransaction& txPrev, const COutPoint& prevout, unsigned int nTimeTx, uint256& hashProofOfStake, uint256& targetProofOfStake, bool fPrintProofOfStake)
{
...
{
    return CheckStakeKernelHash(pindexPrev, nBits, blockFrom.GetBlockTime(), txPrev, prevout, nTimeTx, hashProofOfStake, targetProofOfStake, fPrintProofOfStake);;
}
...
bool CheckProofOfStake(CBlockIndex* pindexPrev, const CTransaction& tx, unsigned int nBits, uint256& hashProofOfStake, uint256& targetProofOfStake)
{
    ...
    // Min age requirement
    int nDepth;
    if (IsConfirmedInNPrevBlocks(txindex, pindexPrev, Params().StakeMinConfirmations() - 1, nDepth))
...
{
        return (nTimeBlock == nTimeTx) && ((nTimeTx & Params().PoSGranularity()) == 0);
}
