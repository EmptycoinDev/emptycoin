...
Value getstakesubsidy(const Array& params, bool fHelp)
{
    ...
    return (uint64_t)GetProofOfStakeReward(pindexBest, Bignum(nCoinAge), 0, pindexBest->nHeight);
}
...
Value getmininginfo(const Array& params, bool fHelp)
{
    ...
    obj.push_back(Pair("stakeinterest",    (uint64_t)Params().StakeCoinYearReward()));
...
Value getstakinginfo(const Array& params, bool fHelp)
{
    ...
    uint64_t nExpectedTime = staking ? (Params().GetTargetSpacing(nBestHeight) * nNetworkWeight / nWeight) : 0;
...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "Emptycoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "Emptycoin is downloading blocks...");

    COutPoint kernel;
    CBlockIndex* pindexPrev = pindexBest;
    unsigned int nBits = GetNextTargetRequired(pindexPrev, true);
    int64_t nTime = GetAdjustedTime();
    nTime &= ~Params().PoSGranularity();
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "Emptycoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "Emptycoin is downloading blocks...");
